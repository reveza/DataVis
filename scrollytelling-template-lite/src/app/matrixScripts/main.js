...
(function (d3, localization) {
  ...
  Promise.all(files.map(url => d3.csv(url))).then(function (results) {
      ...
      addXAxis(vizChartGroup, xAxis, yAxis, height, width);
      getLegend(vizChartGroup, width, height, color, results, null);
      createBubbles(vizChartGroup, dataset, x, y, r);

      //Addition of the force through the creation o the bubble matrix
      var simulationForce = createBubbleMatrix(vizChartGroup, dataset, width, height, r, color, tip, x.domain(), y.domain())
      simulationForce.stop();
      d3.selectAll("text.transmissionTitle").attr("opacity", 0);
      createHistogram(vizChartGroup, dataset, x, y, r, color, tip);
      ...
      toggleButtons.on("click", function(d, i) {
        ...
        currentViz = d3.select(this).text();
          ...
          if(currentViz == "Histogram"){            
            ...
            simulationForce = createBubbleMatrix(vizChartGroup, dataset, width, height, r, color, tip, x.domain(), y.domain());
          }
        });
